 ...
 * 
 * Usage:
 *   node scripts/test-jina-embedding-clean.js [test-case]
 * 
 * Test Cases:
...
const EmbeddingWorker = require('../src/worker/EmbeddingWorker');
...
async function testBasicEmbedding() {
  ...
  try {
    ...
    const embedding = await EmbeddingWorker.generateEmbedding(text);
...
async function testEmptyInput() {
  ...
  try {
    ...
    const embedding = await EmbeddingWorker.generateEmbedding('');
...
async function testLongText() {
  ...
  try {
    ...
    const start = Date.now();
    const embedding = await EmbeddingWorker.generateEmbedding(LONG_TEXT);
    const duration = Date.now() - start;
...
async function testBatchEmbedding() {
  ...
  try {
    ...
    const start = Date.now();
    const embeddings = await EmbeddingWorker.generateEmbedding(TEST_TEXTS);
    const duration = Date.now() - start;
